...
class ConfigManager {
    ...
    async _doLoadConfig() {
        try {
            console.log('📋 正在加载系统配置文件...');
            // 加时间戳并禁止缓存，防止线上仍读到旧配置
            const response = await fetch(`./config.json?t=${Date.now()}`, { cache: 'no-store' });
            
            if (!response.ok) {
                throw new Error(`系统配置文件加载失败: HTTP ${response.status} ${response.statusText}`);
            }
            ...
            throw new Error(`系统配置加载失败: ${error.message}`);
        }
    }
    
    _validateConfig() {
        if (!this.config) {
            throw new Error('系统配置对象为空');
        }
        
        if (!this.config.backend) {
            throw new Error('系统配置文件缺少后端配置');
        }
        
        const { host, port, protocol, wsProtocol } = this.config.backend;
        
        if (!host) {
            throw new Error('系统配置文件缺少后端主机配置');
        }
        
        if (!port) {
            throw new Error('系统配置文件缺少后端端口配置');
        }
        
        if (!protocol) {
            throw new Error('系统配置文件缺少后端协议配置');
        }
        
        if (!wsProtocol) {
            throw new Error('系统配置文件缺少WebSocket协议配置');
        }
        ...
        const errorHtml = `
            ...
            ">
                ...
                ">
                    <h2>⚠️ 系统配置加载失败</h2>
                    <p>${message}</p>
                    <p><strong>请检查 config.json 配置文件是否存在且格式正确！</strong></p>
    ...
    _ensureConfigLoaded() {
        if (!this.isLoaded || !this.config) {
            throw new Error('系统配置未加载，请先调用 loadConfig() 方法');
        }
    }
...
window.configManager.loadConfig().catch(error => {
    console.error('❌ 系统配置加载失败，金融数据系统无法正常工作:', error);
}); 