...
支持在后端基于用户输入的意图自动将当前会话切换到量化专员档位（如 `quant`）。

环境变量：

- `AUTO_ROUTE_QUANT=true`  启用/禁用自动路由（默认 true）
- `AUTO_ROUTE_QUANT_PROFILE_ID=quant`  指定量化档位ID（默认 quant）

逻辑说明：

- 当收到 `user_msg` 且检测到明显的量化关键词（如“量化/策略/回测/因子/恒生/PTrader”等）时，若当前非量化档位，将自动切到量化档位。
- 前端会收到 `model_switched` 事件，`reason=auto_route_quant`，并自动更新下拉按钮与本地记忆。
