...
class ShareApp {
    ...
    async init() {
        try {
            // 从URL参数获取 shareId 或 sessionId（优先 shareId）
            const params = new URLSearchParams(window.location.search);
            this.shareId = params.get('share');
            this.sessionId = params.get('session');
            
            if (!this.shareId && !this.sessionId) {
                this.showError('无效的分享链接', '缺少分享 ID 参数');
                return;
            }
    ...
    getSessionIdFromUrl() { return (new URLSearchParams(window.location.search)).get('session'); }
    getShareIdFromUrl() { return (new URLSearchParams(window.location.search)).get('share'); }
    
    async loadSharedChat() {
        try {
            this.showLoading('正在加载对话内容...');
            ...
            if (!response.ok) {
                if (response.status === 404) {
                    this.showError('\u627e\u4e0d\u5230\u5bf9\u8bdd', '\u672a\u627e\u5230\u6b64\u4f1a\u8bdd\u7684\u804a\u5929\u8bb0\u5f55\uff0c\u53ef\u80fd\u5df2\u88ab\u5220\u9664\u6216\u4f1a\u8bdd ID \u65e0\u6548');
                } else {
    ...
    async loadSnapshotChat() {
        try {
            this.showLoading('正在加载分享快照...');
            const timestamp = Date.now();
            const apiUrl = window.configManager.getFullApiUrl(`/api/share/s/${encodeURIComponent(this.shareId)}?t=${timestamp}`);
            let response;
            try {
                response = await fetch(apiUrl, { cache: 'no-store', mode: 'cors' });
            } catch (e1) {
                const fallbackUrl = `/api/share/s/${encodeURIComponent(this.shareId)}?t=${timestamp}`;
                console.warn('⚠️ 主地址请求失败，尝试同源兜底:', fallbackUrl, e1);
                response = await fetch(fallbackUrl, { cache: 'no-store' });
            }
            if (!response.ok) {
                if (response.status === 404) {
                    this.showError('找不到分享', '分享不存在或已删除');
                } else {
                    throw new Error(`HTTP ${response.status}: ${response.statusText}`);
                }
                return;
            ...
            this.displayChatHistory(result.data);
            document.title = `Shared Conversation - MCP Web Assistant`;
            this.hideLoading();
        } catch (error) {
            console.error('❌ 加载分享快照失败:', error);
            this.showError('加载失败', `无法获取分享快照: ${error.message}`);
        }
    }
