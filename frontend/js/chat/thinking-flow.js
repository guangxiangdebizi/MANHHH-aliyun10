...
class ThinkingFlow {
    ...
    createThinkingFlow() {
        ...
        // 初始默认折叠，但未被用户显式展开
        flowDiv.dataset.userExpanded = '0';
        ...
        this.appInstance.smartScrollToBottom();
    }
    ...
    endThinkingContent(iteration = null) {
        ...
        // 在该阶段结束时渲染 Mermaid 等高级 Markdown 扩展
        try {
            const bubble = thinkingStage && thinkingStage.querySelector('.thinking-content');
            if (bubble && this.appInstance && this.appInstance.messageManager && this.appInstance.messageManager.markdownRenderer) {
                const mr = this.appInstance.messageManager.markdownRenderer;
                if (typeof mr.afterRender === 'function') mr.afterRender(bubble, true);
            }
        } catch {}
    ...
    completeThinkingFlow(status = 'success') {
        ...
        if (status === 'success') {
            thinkingText.textContent = 'Reasoning completed';
            flowHeader.classList.add('completed');
        } else {
            thinkingText.textContent = 'Processing error';
    ...
    checkAllToolsCompleted() {
        ...
        if (allTools.length > 0 && allTools.length === completedTools.length) {
            this.updateThinkingStage('tools_completed', 'Tools execution completed', 'Synthesizing findings and preparing response...');
            // 完成后仅在用户未手动展开时自动折叠
            const flowId = this.currentThinkingFlow.id;
            const flowEl = this.currentThinkingFlow;
            if (!flowEl || flowEl.dataset.userExpanded === '1') return;
    ...
    updateHeaderSummary() {
        ...
        const pieces = [];
        if (total > 0) pieces.push(`Tools ${done}/${total}`);
        if (error > 0) pieces.push(`${error} error`);
        text.textContent = pieces.length ? pieces.join(' · ') : 'Processing...';
    }
    ...
    toggleThinkingFlow(flowId, forceCollapse = false) {
        ...
        if (forceCollapse) {
            // 程序化折叠：若用户手动展开过则尊重用户，不折叠
            if (flowDiv.dataset.userExpanded === '1') return;
        ...
        if (!isCollapsed) {
            ...
            // 用户点击触发的展开 - 完全展开，不限制高度
            flowDiv.classList.remove('collapsed');
            content.style.maxHeight = 'none';
            if (toggleIcon) toggleIcon.textContent = '▼';
            flowDiv.dataset.userExpanded = '1';
        }
    }
