...
class ShareModule {
    ...
    async shareChat() {
        ...
        try {
            ...
            // 调用后端创建只读分享快照，返回 share_id
            const apiUrl = window.configManager.getFullApiUrl('/api/share/create');
            ...
            const res = await fetch(apiUrl, {
                ...
                body: JSON.stringify({ session_id: this.chatApp.sessionId, limit: 200 })
            });
            const data = await res.json().catch(() => ({}));
            if (!res.ok || data.success === false || !data.share_id) {
                throw new Error(data.detail || data.message || `HTTP ${res.status}`);
            }
            const base = window.location.origin + window.location.pathname;
            const shareUrl = `${base}?share=${encodeURIComponent(data.share_id)}`;
            this.showShareDialog(shareUrl);
        } catch (e) {
            console.error('创建分享失败，回退到 session 直链:', e);
            // 回退旧方案（不推荐）：直接按 session 构建链接
            const base = window.location.origin + window.location.pathname;
            const fallbackUrl = `${base}?session=${encodeURIComponent(this.chatApp.sessionId)}`;
            this.showShareDialog(fallbackUrl);
        }
    }
            ...
            <div class="share-modal-content">
                ...
                <div class="share-modal-body">
                    ...
                    <div class="share-notice">
                        ...
                        <ul>
                            <li>分享的对话为只读模式，他人无法继续对话</li>
                            <li>请确保对话内容不包含敏感信息</li>
                            <li>分享链接包含完整对话历史记录</li>
                        </ul>
                    </div>
                </div>
            </div>
